public class GrausGauß extends MiniJava {
  ...
  // Test-Methode um di einzelnen Methoden zu testen
  ...
  public static void printMatrix(int[] matrix) {
    ...
    // Hier werden die Anzahl der Stellen der einzelnen Zahlen ausgerechnet
    // und in ein Array geschrieben
    // Außerdem wird nach der jeweils größten Anzahl der Stellen pro Spalte geschaut
    // Ist nur für eine schönere Ausgabe der Matrix
    ...
    for (int i = 0; i < matrix.length; i++) {
      ...
      // Zwischenraum mit Leerzeichen füllen
  ...
  public static int kgv(int a, int b) {
    // Aus Wikipedia
    // Berechnung des kgV über den ggT mithilfe des modernen euklidischen Algorithmus
    int h;
    int ggT = a;
    int bT = b;
    while (bT != 0) {
  ...
  public static int[] rowEchelonToResult(int[] matrix) {
    ...
    // Fang bei der untersten Zeile an
    for (int i = lines - 1; i >= 0; i--) {
      int res = get(matrix, i, lines);
      // Ziehe alle bisherigen Lösungen multipliziert mit deren Koeffiezient von der Lösung ab
      for (int j = i + 1; j < resArr.length; j++) {
        res -= resArr[j] * get(matrix, i, j);
      }
      // Teile durch den Koeffizienten
