...
public class Interpreter extends MiniJava {
  ...
  public static final int LDH = 25;
  public static final int STH = 26;
  public static final int ALLOCH = 27;
  ...
  private static void alloch() {
    ...
    if(heap[heap.length-1] != heap.length -1) {
      lTo = (heap[heap[heap.length-1]] & (0xFFFF << 16)) >> 16;
    }

    int o1 = pop();

    if(o1 < 0 || heap[lTo]+o1 > heap[heap.length-1]-1){
      error("Konnte kein Objekt mit Größe " + o1 + " auf dem Heap anlegen");
    }
    ...
    heap[heap[heap.length-1]] = (to << 16) | from;

    System.out.println("Objekt mit Größe " + o1 + " angelegt (" + ((heap.length-1)-heap[heap.length-1]) + ")");

    push((heap.length-1)-heap[heap.length-1]);

    System.out.println("Stack Pointer: " + sp);
  }

  private static void sth() {
    heap[getHeapIndex(pop(), pop())] = pop();
  }

  private static void ldh() {
    push(heap[getHeapIndex(pop(), pop())]);
  }

  private static int getHeapIndex(int o1, int o2){
    if(o1 < 1 || o1 > (heap.length-1)-heap[heap.length-1]){
      error("Auf das " + o1 + "-te Objekt konnte nicht zugegriffen werden");
    }
    int header = heap[(heap.length-1)-o1];
    int from = header & 0xFFFF;
    int to = (header & (0xFFFF << 16)) >> 16;

    if(o2 < 0 || o2 >= to-from){
      error("Auf den Offset " + o2 + " von Objekt " + o1 + " konnte nicht zugegriffen werden");
    }
  ...
  private static void alloc(int imm, int prevOpCode) {
    if (prevOpCode != CALL && prevOpCode != -1) {
      error("Alloc kann nur am Anfang einer Funktion oder des Programms stehen (" + currInstr + ")");
    }
    for (int i = 0; i < imm; i++) {
      push(0);
    }
