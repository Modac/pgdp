...
public class MiniJavaParser {
  ...
  public Declaration parseDecl() {
    ...
    // ;
    checkToken(";");
    return new Declaration(type, names.toArray(new String[]{}));
  ...
  public Expression parseExpression() {
    ...
    // oder eine weitere Expression in eckigen Klammern
    if (isExpr) {
      from = fromF;
      ...
      if (from > 0) {
        ...
        checkToken("]");
        if (from > 0) {
          return new ArrayAccess(expr, expr3);
        } else {
          from = fromF;
          return expr;
        }
  ...
  public Statement parseStatement() {
    ...
    if (from > 0) {
      ...
      if (from > 0) {
        ...
        checkToken(";");
        if( from > 0){
            return new ArraySet(varName, expr1, expr2);
        }
      }
    }
  ...
  public Function parseFunction() {
    ...
    checkToken("}");
    if(from > 0 ){
      return new Function(type, name, params, decls.toArray(new Declaration[]{}),
          statemnts.toArray(new Statement[]{}));
    }
    return null;
  }

  public Parameter[] parseParams() {
    ...
    // (<type> <name>)
    Type type = parseType();
    params.add(new Parameter(type, parseName(), params.size()));
    if (from < 0) {
      from = fromO;
